...
class Product(Toplevel):     
    ...
    def set_values(self,):
        ...
        key = next(key for key, value in self.dict_brands.items() if value == self.selected_product[2])
        self.cbBrands.current(key)
        ...
        self.available.set(self.selected_product[7])

    def get_values(self,):
        return (self.txtProduct.get(),
                self.dict_brands[self.cbBrands.current()],
    ...
    def set_brands(self,):
        ...
        sql = "SELECT brand_id, company FROM brands ORDER BY company ASC"
        rs = self.engine.read(True, sql, ())
